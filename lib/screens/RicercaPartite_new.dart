...
class RicercaPartite_New extends StatefulWidget {
  Giocatore giocatore;
  RicercaPartite_New({required this.giocatore, Key? key}) : super(key: key);

  @override
  _RicercaPartite_New createState() => _RicercaPartite_New();
}

class _RicercaPartite_New extends State<RicercaPartite_New> {
        ...
        try {
          for (var i = 0; i < partiteconfermate.length; i++)
            for (var k = 0; k < giocatore.partiteconfermate!.length; k++)
              if (partiteconfermate[i].id.key ==
                  giocatore.partiteconfermate![k].id.key)
